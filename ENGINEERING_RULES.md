...
**Rule 4.13 — Derived, on demand.** A derived artifact (extracted subtitles, a keyframe map, trickplay images) is built when a consumer needs it. A whole-library pass over every item is an explicitly requested operation, never a default consequence of scanning. Rule 4.9 still decides the artifact's shape before its writer exists; this rule decides when the writer runs. Illustration: on 2026-08-06 the same mistake shipped three times independently — probe already classified which items needed subtitle extraction and the classification was discarded in favor of a rediscovery queue, subtitle extraction queued library-wide, and the keyframe map queued library-wide — and two of those whole-library queues sharing one scheduling class took the household array down. An eager whole-library pass stays available as an opt-in operator action (Rule 4.12's hardware escape hatch), not as the default path.

**Rule 4.14 — A number carries its provenance, and its reach.** A constant, threshold or pass criterion that governs behaviour states where it came from, at the line that defines it: the measurement that produced it, the reasoning that derived it, or the fact that it is a guess. A measurement also states what it was measured on, and a number never outgrows that. A figure taken on one machine, one container, or the dogfood library is evidence about that machine and that library — it is not a global default. Nightjar runs on hardware we do not own and libraries we have not seen, and the household box is the easiest case, not the representative one; such a number is re-derived for the population it will govern, or expressed as something the server measures at runtime. A superseded decision does not survive in code because nobody deleted it: when an ADR is superseded, the constants, env vars and code paths it named are retired in the same change or an issue owns their removal, and a number whose only ancestry is a withdrawn measurement does not constrain new work — it is investigated and surfaced for a decision, never obeyed by default. Illustration: `DEFAULT_MAX_ENCODERS = 3` came from ADR-0007's "Gate 2 N100 figure"; ADR-0050 §6–§8 superseded that model and withdrew the number, and the constant went on refusing a fourth stream on every box, including hardware that measured seven concurrent sessions cleanly. The same 3 was written a second time, uncommented, in `api/src/main.rs`, because the library constant is private — so correcting one would not have changed what ships. `MAX_EPISODE_RANGE = 8` sits beside a comment reading "Dogfood max is 3": one household's library setting a parser bound for everyone else's.
**Rule 4.15 — A criterion that cannot fail is not evidence.** Where a check passes on zero, empty, or absence, something must prove the instrument was live: a positive control, a high-water mark asserted above zero, or a count of what it did see. An instrument that has never been observed reporting the failing value has not been shown to work, and a gate does not close on it. A guard whose own failure mode is a pass is the same defect — a command substitution yielding an empty string on error, a `grep -c` swallowed by `|| true`, a parse returning an empty map on a format change. Illustration: the Gate 2 orphan soak passes on zero orphaned processes, and six independent times it reported zero because it could not see — a private PID namespace, a wrong data dir, and four more. The run of 2026-09-04 churned 538 sessions and recorded `ffmpegCount: 0` on all 1,072 samples while the server's own log showed `mode=Transcode encoder=h264_qsv` and real segment output. The field that would have caught every one of the six was already in the summary and was never asserted above zero.
