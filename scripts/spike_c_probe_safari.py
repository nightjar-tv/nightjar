#!/usr/bin/env python3
"""Spike C — Safari native HLS via safaridriver."""
...
OUT = os.environ.get("SPIKE_OUT", "/tmp/spike_c_safari.json")
PORT = int(os.environ.get("SPIKE_SAFARI_PORT", "19653"))
PAGE = f"{BASE}/index.html?engine=native&master={MASTER}&spliceS={SPLICE_S}"
...
def main() -> int:
    ...
    try:
        ...
        for _ in range(40):
            ...
            raise SystemExit(f"safaridriver session failed: {last}")
        ...
        with open(OUT, "w") as f:
            ...
            f.write("\n")
        print(
            f"safari {VARIANT}: crossed={payload.get('crossed')} "
