...
export interface paths {
    ...
    "/api/v0/items/{itemId}/sessions": {
         ...
         * @description ADR-0011 / ADR-0020. Returns 202 with sessionId and playlistUrl (the session master, /sessions/{sessionId}/master.m3u8). Remux items get a stream-copy session, transcode items a re-encoding one; the shape is identical. Far seek uses POST /sessions/{sessionId}/seek?startMs=, which returns the same playlistUrl (ADR-0054 decision 5). Admission is measured, not configured (ADR-0050 sections 7 and 8): a session is admitted unless the box has too little memory to hold another encoder. There is no default concurrency limit. NIGHTJAR_HLS_MAX_ENCODERS remains available as an operator override.
         */
