<script lang="ts">
	...
	/**
	 * The surface outlives the session. `false` shows the play affordance;
	 * a session is a transient server resource with an idle reap (ADR-0007 §4)
	 * and losing one must not cost the viewer the page they are on.
	 */
	let started = $state(false);
	/** Where a press of play would resume from. */
	...
	/**
	 * Intent to play, set by the item page's play button. Its absence is what
	 * makes a bookmark or a reopened browser land on the affordance instead of
	 * starting a session nobody asked for.
	 */
	const playIntent = $derived(page.url.searchParams.get('play') === '1');
	...
	/**
	 * The session behind the current attach is gone (idle reap, ADR-0007 §4, or
	 * a server restart). Drop back to the play affordance at the last known
	 * position rather than retrying, erroring, or navigating away.
	 *
	 * Nobody accepts a television app that returns you to the show page after
	...
	function onSessionGone() {
		...
		// No DELETE: the server already dropped it, so the request would only
		// be another 404 against a session that does not exist.
		sessionRef.id = null;
		sessionEncoder = null;
		playlistUrl = null;
		started = false;
	...
	/** Direct play has no session and no window offset: media time is title
	 *  time, so resume is a seek and position is `currentTime`. */
	function onDirectPlayReady() {
		if (!videoEl || resumeMs <= 0) return;
		videoEl.currentTime = resumeMs / 1000;
		void videoEl.play().catch(() => {
			// Autoplay policy: the element's own controls still work.
		});
	}

	function onDirectPlayTime() {
		if (!videoEl) return;
		scrubSec = videoEl.currentTime;
		rememberPositionMs(itemId, videoEl.currentTime * 1000);
	}

	/** Create a session (or attach direct play) and begin. */
	async function start() {
		...
		preparingSession = false;
		// Producer EOF may set usableExtentMs after the POST returns (damaged
		// mid-title). Poll until known or the page dies.
		void (async () => {
	...
	onMount(() => {
		...
		(async () => {
			...
			// Explicit deep link wins over anything remembered; otherwise the
			// one position read (ADR-0035 / B2-3 fills its cold path).
			resumeMs = startMsFromSearch || ((await resumePositionMs(itemId)) ?? 0);
			if (!liveRef.alive) return;
			if (playIntent) await start();
		})().catch((e: Error) => {
			preparingSession = false;
			error = e.message;
		});
	...
	$effect(() => {
		...
		const onTime = () => {
			...
			// The position a dead session resumes from, kept without asking
			// the server (which has nowhere to store it until B2-3).
			rememberPositionMs(itemId, handle.positionSeconds() * 1000);
		};
...
<main>
	...
	{#if item && playback}
		...
		{#if !started}
			<!-- The surface persists, the session does not. Arriving cold, or
		...
		{:else if playable && playback.streamUrl}
			...
			<video
				...
				onloadedmetadata={onDirectPlayReady}
				ontimeupdate={onDirectPlayTime}
			>
