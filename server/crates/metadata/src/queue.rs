...
use crate::item_links::series_key_for_show_folder;
...
pub(crate) fn series_show_id_for_folder(
    ...
    // mean "not bound" to every caller here (ADR-0039 item 3).
    Ok(conn
        .query_row(
...
fn upsert_series_row(conn: &Connection, g: &QueryGroup, show_id: i64) -> Result<(), String> {
    ...
    let old_key = series_key_for_show_folder(g.library_id, &g.show_folder, previous);
    let new_key = series_key_for_show_folder(g.library_id, &g.show_folder, Some(show_id));
    crate::migrator::migrate_series_keys(conn, &old_key, &new_key)?;
    Ok(())
}
...
fn status_query_groups(
    ...
    for it in &items {
        ...
        match it.kind.as_str() {
            ...
            "episode" => {
                ...
                let unit_key = series_key_for_show_folder(
                    folder_key.0,
                    &folder_key.1,
    ...
    // formation happens and `folder_key` is already the row's primary key, so
    // migration, queue and rollup cannot disagree about what a folder is.
    //
    ...
    // fact that arrives later or never.
    ensure_series_rows(conn, ep_groups.keys())?;
...
fn ensure_series_rows<'a>(
    conn: &Connection,
    folders: impl Iterator<Item = &'a (i64, String)>,
) -> Result<(), String> {
    let mut stmt = conn
        .prepare(
