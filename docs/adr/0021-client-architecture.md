...
- Status: accepted
- Date: 2026-07-31
- Accepted: 2026-08-01 (stack and Rule 2.4 amend); Apple engine closed to
  option (a), AetherEngine, on 2026-08-06
- Supersedes: ADR-0001 (pending stub); prior constitution wording that required
  one shared Rust/libmpv player core (§1 stack table / Rule 2.4)
- Evidence: `notes/client-arch/engine-bakeoff.md` (bake-off branch /
  measurement corpus)
- Constitution: ENGINEERING_RULES.md amended under Rule 6.4 in the same change
...
|---|---|---|---|---|
| macOS, iOS | Flutter (stock) | AetherEngine (option (a)) | Direct play when the profile allows; sessions for bandwidth / burn-in / outliers | Decided |
| tvOS | Flutter only if flutter-tvos stays healthy; else native SwiftUI | AetherEngine (option (a)) | Direct play and/or session per engine | Engine decided; shell tied to flutter-tvos health |
...
### Apple engine: option (a), AetherEngine

**Closed 2026-08-06 in favour of (a).** Per-platform engines exist to maximise
direct play, and on Apple that is what separates the two options: (b)'s living
room leg had no Matroska-capable engine that was not either Aether anyway or a
Nightjar-owned media_kit tvOS port the bake-off scores as a T3 breach. One
engine and one capability profile across every Apple device, `AETHER_V0` ships,
and the ~48k Swift lines and single-maintainer exposure are accepted with the
fallback below written down.

The bake-off open items (Part C on Apple TV hardware, Aether full n=228 T4,
measured Flutter platform-view glue cost in the Nightjar tree) are **not**
reopened by this and stay Phase 4 measurement work. They would change how well
(a) performs, not which option is chosen.

The two options as they were analysed, kept as the record:
...
This ADR determines which profiles exist; the Step 2 profile ADR
(ADR-0022) defines bitrate / resolution / HDR fields and wire shape:
...
| `AETHER_V0` | Apple (option (a), shipping). Still **unscored**: needs a `t1_profile_counts.py` run before Gate 1 language is reused, and it stays unmeasured for per-profile HDR/DV render (`corpus_decide.rs` prints this as provisional under Rule 4.8) |
...
**One Rust/libmpv core on every platform (constitution as previously written).**
Would keep one bug surface. Rejected: Media3 already clears Android without
it; stock Flutter has no tvOS target and media_kit has no `tvos:` plugin;
Tizen/webOS ABI and vendor-player policy make a Nightjar engine on those TVs
a Rule 4.4 and T3 mistake. Keeping the wording would force fiction on
platforms we will ship.
...
- Rule 2.4's intent (fix playback once per bug class) moves to the Dart
  ...
  ENGINEERING_RULES.md §1 and Rule 2.4 were amended under Rule 6.4 to match.
- Android ships on evidence already in hand.
- Windows/Linux share media_kit with a clear profile (`MPV_V0`).
- Apple is one engine and one capability profile across macOS, iOS and tvOS
  under (a), so there is no Apple-internal engine split to carry, and stock
  AVPlayer remux was never the TV answer.
- Sessions and full-title VOD are funded as product work for every
  non-direct-play client.
- Rule 2.6 (client matrix) is drafted so "works on one engine" cannot be
  mistaken for done. It is **not** in the constitution yet: it waits on the
  `docs/rule-2-6-all-client-release-gate` branch, and this ADR does not land it.

**Bad (accepted)**

- Three engine families to debug at steady state (Media3, libmpv, Aether)
  instead of one Rust core, plus the vendor players on Tizen and webOS.
- Aether is Apple-only, roughly 48k Swift lines under a single maintainer, and
  LGPL-3.0 with an App Store / DRM exception. That concentration is the price
  of (a) and the fallback below is the answer to it, not a reason to reopen.
...
**Still open (not blocked by the constitution amend)**

- Bake-off Part C, Aether n=228 T4, and Jellyfin T5-4 comparisons remain Phase 4
  measurement work. The Apple engine choice is closed and is not among them.
- Step 2 profile ADR (ADR-0022) + N100 capacity remain Phase 2 / Gate 2.
- No T-gate threshold from the bake-off is moved by this acceptance.
